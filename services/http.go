...
func CheckAccountWithContext(ctx context.Context, ssoCookie string, userID string, captchaAPIKey string) (models.Status, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "GET", checkRequest, nil)
	if err != nil {
		return models.StatusUnknown, fmt.Errorf("failed to create HTTP request: %w", err)
	}
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		select {
		case <-ctx.Done():
			return models.StatusUnknown, fmt.Errorf("context cancelled during HTTP request")
		default:
			logger.Log.Infof("Sending HTTP request to check account (attempt %d/%d)", attempt, maxRetryAttempts)
			resp, err = client.Do(req)
			if err != nil {
				if attempt == maxRetryAttempts {
					return models.StatusUnknown, fmt.Errorf("failed to send HTTP request after %d attempts: %w", maxRetryAttempts, err)
				}
			...
			if err != nil {
				if attempt == maxRetryAttempts {
					return models.StatusUnknown, fmt.Errorf("failed to read response body after %d attempts: %w", maxRetryAttempts, err)
				}
				time.Sleep(time.Duration(attempt) * time.Second)
				continue
			}
