...
func RegisterCommand(s *discordgo.Session, guildID string) {
	...
	if err != nil {
		logger.Log.WithError(err).Error("Error creating accountlogsnew command")
	}
}
...
func UnregisterCommand(s *discordgo.Session, guildID string) {
	...
	for _, command := range commands {
		if command.Name == "accountlogsnew" {
			err := s.ApplicationCommandDelete(s.State.User.ID, guildID, command.ID)
			if err != nil {
				logger.Log.WithError(err).Error("Error deleting accountlogsnew command")
			}
			return
		}
	}
...
func CommandAccountLogsNew(s *discordgo.Session, i *discordgo.InteractionCreate) {
