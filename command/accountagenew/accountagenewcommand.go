...
func RegisterCommand(s *discordgo.Session, guildID string) {
	...
	if err != nil {
		logger.Log.WithError(err).Error("Error creating accountagenew command")
	}
}
...
func UnregisterCommand(s *discordgo.Session, guildID string) {
	...
	for _, command := range commands {
		if command.Name == "accountagenew" {
			err := s.ApplicationCommandDelete(s.State.User.ID, guildID, command.ID)
			if err != nil {
				logger.Log.WithError(err).Error("Error deleting accountagenew command")
			}
			return
		}
	}
...
func CommandAccountAgeNew(s *discordgo.Session, i *discordgo.InteractionCreate) {
